function Mine(tr, td, mineNum) {
    ...
    this.messageDiv.appendChild(this.messageContent);
    document.body.appendChild(this.messageDiv);

    // 弹窗显示函数
    this.showMessage = function (text) {
        this.messageContent.innerHTML = `<h2>${text}</h2>`;
        const okButton = document.createElement('button');
        okButton.textContent = 'OK';
        okButton.onclick = () => this.closeMessage(); 
        this.messageContent.appendChild(okButton);
...
Mine.prototype.createDom = function () {
    ...
    for (var i = 0; i < this.tr; i++) { // 行
        ...
        for (var j = 0; j < this.td; j++) { // 列
            ...
            domTd.onmousedown = function (ev) {
                This.play(ev, this); // 大的This指的是实例对象   小的this指的是点击的domTd 
            };
...
Mine.prototype.play = function (ev, obj) {
    ...
    if (ev.which == 3) {
        ...
        if (this.surplusMine == 0) {
            ...
            if (this.allRight) {
                for (i = 0; i < this.tr; i++) {
                    this.showMessage('恭喜你摸鱼的天才，游戏成功！');
                    for (j = 0; j < this.td; j++) {
                ...
                this.showMessage('怎么摸鱼的？游戏失败了T^T');
                this.gameOver();
            }
        }
    }
    ...
    */
    for (i = 0; i < this.tr; i++) {
        for (j = 0; j < this.td; j++) {
            ...
            this.tds[i][j].onmousedown = null;//禁用点击事件
        }
    }
    if (clickTd) {
        clickTd.style.backgroundColor = '#f00';//标记点击的雷
        this.showMessage('坏了，摸鱼出事故了，你踩到地雷了！');
    }
};
